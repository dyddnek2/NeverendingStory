...
export class StateManager {
  ...
  private static defaultAuthorIntent(language: "ko" | "zh" | "en"): string {
    return language === "zh"
      ? "# 作者意图\n\n（在这里描述这本书的长期创作方向。）\n"
      : language === "ko"
  ...
  private static defaultCurrentFocus(language: "ko" | "zh" | "en"): string {
    return language === "zh"
  ...
  private async resolveControlDocumentLanguage(bookId: string): Promise<"ko" | "zh" | "en"> {
    try {
      const raw = await readFile(join(this.bookDir(bookId), "book.json"), "utf-8");
      const parsed = JSON.parse(raw) as { language?: unknown };
      return parsed.language === "zh" || parsed.language === "ko" ? parsed.language : "en";
    } catch {
      return "en";
    }
  }
